...
for item in files:
        ...
        for i in range(len(sentence_li)-2, -2, -2):
                del sentence_li[i]

        #Xu li content


        for n in range(len(sentence_li)):
        ...
        for n in range(len(sentence_li)):
                ...
                while sentence.find('<') != -1 and sentence.find('>') != -1:
                        i = sentence.index('<')
                        j = sentence.index('>')
                        sentence = sentence[:i].strip() +' ' +sentence[(j+2):].strip()
        ...
        for n in range(len(sentence_li)):
                ...
                while sentence.find(']') != -1 and sentence.find('[') != -1 and sentence.find('+') != -1:
                        ...
                        j = sentence.index('+')
                        t = sentence.index(']')
                        sentence = sentence[:(i+1)].strip() + ' ' + sentence[(j+2):].strip()
                sentence = sentence.replace('[', ' ').replace(']', ' ')
                sentence_li[n] = sentence.strip('-').replace('  ', ' ')
        ...
        for n in range(len(sentence_li)):
                ...
                while n+1 < len(sentence_li) and person_li[n+1] == person_li[n]:
